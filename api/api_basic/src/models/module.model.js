...
class ModuleModel {
  constructor(id, name, description, route, icon, submodule, parent_module, created_at, updated_at) {
    this.id = id;
    this.name = name;
...
/**
 * The function `showModel` fetches all users from a database and returns them as a JSON response,
 * handling errors appropriately.
 * @param res - The `res` parameter in the `showModel` function is typically the response object in a
 * Node.js application. It is used to send the HTTP response back to the client making the request.
 */
  async showModel(res) {
    try {
      let sqlQuery = "SELECT * FROM `modules` WHERE 1;";
 ...
 /**
  * The function `showModelById` retrieves a user from a database by their ID and returns the user data
  * if found, or an error message if not found or an error occurs.
  * @param res - The `res` parameter in the `showModelById` function is typically used to send the HTTP
  ...
  * headers, parameters, body, and more. In this specific function `showModelById`, `req` is likely used
